...

{
  ...
  "horizon_3yr": "2 sentences about their profession by 2028. Include a concrete stat or percentage where possible.",
  "horizon_5yr": "2 sentences about their profession by 2030. Include a concrete stat or percentage where possible.",
...
export async function generatePDF(data) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Explanation: max 2 lines, explicitly constrained
    const expl = (narratives.archetype_explanation || '').slice(0, 160);
    doc.font('Helvetica').fontSize(9.5).fillColor('rgba(245,242,236,0.6)')
       .text(expl + (narratives.archetype_explanation.length > 160 ? '…' : ''),
             M.left + 16, boxY + 56, { width: UW - 32, height: 50, lineGap: 2, lineBreak: true });
    ...
    if (narratives.course_recommendation) {
      ...
      const crY = doc.y;
      doc.roundedRect(M.left, crY, UW, 44, 8).fill(hex('#EEF5F1'));
      doc.rect(M.left, crY, 4, 44).fill(hex(C.sage));
      doc.font('Helvetica-Bold').fontSize(8).fillColor(hex(C.sage))
