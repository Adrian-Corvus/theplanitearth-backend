...

{
  ...
  "horizon_3yr": "1 specific sentence about their profession by 2029.",
  "horizon_5yr": "1 specific sentence about their profession by 2031.",
  ...
  "income_insight": "1 sentence on income trajectory for this specific profession with a concrete % or salary estimate.",
  "tool_1_name": "Name of the #1 AI tool this exact profession should learn right now (e.g. Harvey, Copilot, Otter.ai, Midjourney)",
  "tool_1_why": "One sentence: exactly why this specific tool matters for their job tasks",
  "tool_2_name": "Name of the #2 AI tool for this profession",
  "tool_2_why": "One sentence: exactly why this specific tool matters for their job tasks",
  "tool_3_name": "Name of the #3 AI tool for this profession",
  "tool_3_why": "One sentence: exactly why this specific tool matters for their job tasks",
  "course_recommendation": "One specific course or cert with provider (e.g. 'AI for Finance Professionals — Coursera / University of Pennsylvania')",
  "horizon_1yr": "2 sentences about their profession in the next 12 months. Include a concrete stat or percentage where possible.",
  "horizon_3yr": "2 sentences about their profession by 2029. Include a concrete stat or percentage where possible.",
  "horizon_5yr": "2 sentences about their profession by 2031. Include a concrete stat or percentage where possible.",
  "closing_advice": "3 sentences. Specific, actionable, forward-looking. Reference their actual job title. End with one concrete first step they can take this week."
}`;
  ...
  try {
    ...
    return parseModelJson(text, 'career brief response');
...
function getTaskData(sector, jobTitle) {
  ...
  const roleProfiles = [
    {
      match: ['optical engineer', 'photonics', 'laser', 'optics', 'fiber optics', 'fibre optics', 'optoelectronics'],
      risk: ['Routine optical simulation runs', 'Standard test-data post-processing', 'First-pass experiment documentation', 'Basic component-performance reporting'],
      augmented: ['Physical experiment design and setup', 'Interpreting noisy lab results and failure modes', 'Prototype integration with real-world hardware constraints', 'Turning measurements into engineering decisions']
    },
    {
      match: ['software engineer', 'developer', 'frontend', 'backend', 'full stack', 'full-stack'],
      risk: ['Boilerplate feature implementation', 'Routine test-case generation', 'Basic API and documentation drafting', 'Template-level bug fixes'],
      augmented: ['System architecture tradeoffs', 'Debugging production edge cases', 'Translating ambiguous stakeholder needs into robust systems', 'Technical leadership and design reviews']
    },
    {
      match: ['data scientist', 'data analyst', 'machine learning', 'ml engineer', 'ai engineer'],
      risk: ['Routine data cleaning and SQL drafting', 'Standard dashboard generation', 'Baseline model experimentation', 'First-pass reporting summaries'],
      augmented: ['Framing the right analytical question', 'Evaluating model reliability and bias in context', 'Turning findings into decisions stakeholders will trust', 'Designing measurement and experimentation strategy']
    },
    {
      match: ['financial analyst', 'accountant', 'auditor', 'investment analyst'],
      risk: ['Recurring reconciliations and spreadsheet prep', 'Standard variance reporting', 'First-pass financial modelling', 'Routine compliance checks'],
      augmented: ['Judgment on unusual transactions or market context', 'Client or stakeholder communication', 'Strategic capital allocation recommendations', 'Interpreting risk beyond the spreadsheet']
    },
    {
      match: ['nurse', 'doctor', 'physician', 'surgeon', 'radiographer', 'therapist'],
      risk: ['Standard note drafting and admin follow-up', 'Protocol-driven triage support', 'Routine image or results pre-screening', 'Template referral letters'],
      augmented: ['Patient communication and trust-building', 'Complex diagnosis and escalation judgment', 'Coordinating care across disciplines', 'Responding to atypical clinical presentations']
    },
    {
      match: ['lawyer', 'solicitor', 'attorney', 'paralegal', 'compliance'],
      risk: ['Template contract review', 'Routine legal research summarisation', 'Standard clause drafting', 'Discovery document triage'],
      augmented: ['Negotiation strategy in complex matters', 'Novel legal interpretation', 'Client judgment and risk counsel', 'Advocacy in high-stakes contexts']
    },
    {
      match: ['teacher', 'lecturer', 'professor', 'trainer', 'tutor'],
      risk: ['First-draft lesson materials', 'Routine marking and feedback summaries', 'Administrative progress reporting', 'Standard resource generation'],
      augmented: ['Live teaching adaptation to student needs', 'Mentoring and pastoral support', 'Designing learning experiences for mixed abilities', 'Facilitating deep understanding and critical thinking']
    },
    {
      match: ['designer', 'ux', 'ui', 'graphic', 'copywriter', 'brand'],
      risk: ['First-draft creative variations', 'Template-based asset production', 'Basic campaign copy generation', 'Routine social content production'],
      augmented: ['Original concept direction', 'Client discovery and creative judgment', 'Taste, positioning, and brand coherence', 'Turning business goals into resonant creative strategy']
    },
  ];

  const roleProfile = roleProfiles.find(profile =>
    profile.match.some(term => title.includes(term))
  );
  ...
  const tasks = {
    ...
    tech: {
      risk: ['Routine documentation and reporting', 'Standard analysis or testing scripts', 'Template-based technical outputs', 'Repeatable implementation tasks'],
      augmented: ['System-level problem framing', 'Judgment under real-world constraints', 'Cross-functional technical decision-making', 'Complex debugging and tradeoff management']
    },
    legal: {
...
function fitTextToBox(doc, text, options) {
  const {
    width,
    height,
    font = 'Helvetica',
  ...
  doc.font(font).fontSize(fontSize).lineGap(lineGap);
  if (doc.heightOfString(source, { width, lineGap }) <= height) {
    return source;
  }

  const sentences = source.match(/[^.!?]+[.!?]+|[^.!?]+$/g)?.map(s => s.trim()).filter(Boolean) || [source];
  let fitted = '';
  ...
  if (fitted) {
    return fitted.endsWith(ellipsis) ? fitted : `${fitted}${ellipsis}`;
  }
  ...
  while (low <= high) {
    ...
    const snippet = words.slice(0, mid).join(' ').trim();
    const candidate = snippet ? `${snippet}${ellipsis}` : '';

    if (candidate && doc.heightOfString(candidate, { width, lineGap }) <= height) {
      best = candidate;
      low = mid + 1;
    } else {
      high = mid - 1;
...
export async function generatePDF(data) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // ── COVER: all elements at FIXED absolute Y positions — no overflow possible ──

    // Title block: y 140–260
    doc.font('Helvetica-Bold').fontSize(50).fillColor(hex(C.white))
       .text('Career',        M.left, 140, { lineBreak: false });
    doc.font('Helvetica-Bold').fontSize(50).fillColor(hex(C.white))
       .text('Intelligence',  M.left, 194, { lineBreak: false });
    doc.font('Helvetica-Bold').fontSize(50).fillColor(hex(C.white))
       .text('Brief',         M.left, 248, { lineBreak: false });

    // Divider
    doc.rect(M.left, 308, 48, 3).fill(hex(C.sage));

    // Name / profession / date: y 320–375
    doc.font('Helvetica').fontSize(14).fillColor(hex(C.sage))
       .text(`Prepared for ${name}`, M.left, 320, { lineBreak: false });
    ...
    doc.font('Helvetica').fontSize(12).fillColor('rgba(245,242,236,0.6)')
       .text(profShort, M.left, 340, { lineBreak: false });
    doc.font('Helvetica').fontSize(10).fillColor('rgba(245,242,236,0.3)')
       .text(today, M.left, 360, { lineBreak: false });

    // ── ARCHETYPE BOX: fixed y=400, fixed height=120 ──
    const boxY = 400;
    const boxH = 120;
    doc.rect(M.left, boxY, UW, boxH).fill(hex('#172214'));
    doc.rect(M.left, boxY, 4, boxH).fill(hex(C.sage));

    doc.font('Helvetica-Bold').fontSize(8).fillColor(hex(C.sage))
       .text('YOUR CAREER ARCHETYPE', M.left + 16, boxY + 12, { characterSpacing: 1.8, lineBreak: false });

    const archShort = archetype.name.length > 36 ? archetype.name.slice(0,34)+'…' : archetype.name;
    doc.font('Helvetica-Bold').fontSize(20).fillColor(hex(C.white))
       .text(archShort, M.left + 16, boxY + 28, { lineBreak: false });

    // Fit the cover blurb to the available box height without cutting mid-line.
    const coverExpl = fitTextToBox(doc, narratives.archetype_explanation, {
      width: UW - 32,
      height: 50,
      font: 'Helvetica',
      fontSize: 9.5,
    ...
    doc.font('Helvetica').fontSize(9.5).fillColor('rgba(245,242,236,0.6)')
       .text(coverExpl, M.left + 16, boxY + 56, { width: UW - 32, height: 50, lineGap: 2, lineBreak: true });

    // ── SCORE STRIP: fixed y=550, well below box bottom (400+120=520) ──
    const stripY = 555;
    const sColW  = UW / 4;

    doc.moveTo(M.left, stripY - 10).lineTo(M.left + UW, stripY - 10)
       .strokeColor(hex('#2A3525')).lineWidth(1).stroke();
    ...
    metrics.forEach((m, i) => {
      const x = M.left + i * sColW;
      doc.font('Helvetica-Bold').fontSize(30).fillColor(hex(m.color))
         .text(`${Math.round(m.val)}`, x, stripY, { width: sColW, align: 'center', lineBreak: false });
      doc.font('Helvetica').fontSize(8).fillColor('rgba(245,242,236,0.38)')
         .text(m.label, x, stripY + 38, { width: sColW, align: 'center', lineBreak: false });
      if (i < 3) {
        doc.moveTo(x + sColW, stripY).lineTo(x + sColW, stripY + 52)
           .strokeColor(hex('#2A3525')).lineWidth(1).stroke();
      }
    });

    // Footer: fixed y=790
    doc.font('Helvetica').fontSize(7.5).fillColor('rgba(245,242,236,0.18)')
       .text('theplanitearth.com  ·  Data: WEF · McKinsey · O*NET · BLS · Brookings',
    ...
    horizons.forEach((h, i) => {
      // Measure text height first so the box is always tall enough
      const textH = doc.heightOfString(h.text, { width: UW - 28, fontSize: 10.5 });
      const rowH  = Math.max(70, textH + 46); // label(20) + sublabel(14) + gap(12) + text + padding
      checkPageBreak(doc, rowH + 8, M);

      const hY  = doc.y;
      const hBg = i % 2 === 0 ? C.bg2 : C.white;
      doc.rect(M.left, hY, UW, rowH).fill(hex(hBg));
      doc.rect(M.left, hY, 4, rowH).fill(hex(h.color));

      doc.font('Helvetica-Bold').fontSize(11).fillColor(hex(h.color))
         .text(h.label, M.left + 14, hY + 10, { lineBreak: false });
      doc.font('Helvetica-Bold').fontSize(8).fillColor(hex(C.mid))
         .text(h.sub.toUpperCase(), M.left + 14, hY + 26, { characterSpacing: 0.8, lineBreak: false });
      doc.font('Helvetica').fontSize(10.5).fillColor(hex(C.ink))
         .text(h.text, M.left + 14, hY + 42, { width: UW - 28 });
    ...
    // ── RECOMMENDED TOOLS ──
    sectionBanner(doc, 'AI Tools to Learn — Specific to Your Role', M, UW, C.sage);

    const tools = [
      { name: narratives.tool_1_name, why: narratives.tool_1_why, rank: '1' },
      { name: narratives.tool_2_name, why: narratives.tool_2_why, rank: '2' },
      { name: narratives.tool_3_name, why: narratives.tool_3_why, rank: '3' },
    ];
    tools.forEach((tool, i) => {
      if (!tool.name) return;
      // Set font BEFORE measuring — PDFKit uses current font state for heightOfString
      doc.font('Helvetica-Bold').fontSize(12);
      const nameH = doc.heightOfString(tool.name || '', { width: UW - 60 });
      doc.font('Helvetica').fontSize(10);
      const whyH  = doc.heightOfString(tool.why  || '', { width: UW - 60 });
      // 10 (top pad) + name + 6 (gap) + why + 14 (bottom pad)
      const tH = Math.max(56, 10 + nameH + 6 + whyH + 14);
      checkPageBreak(doc, tH + 6, M);
      const tY  = doc.y;
      const tBg = i % 2 === 0 ? C.bg2 : C.white;

      doc.rect(M.left, tY, UW, tH).fill(hex(tBg));

      // Rank bubble — vertically centred
      doc.circle(M.left + 22, tY + tH / 2, 14).fill(hex(C.sage));
      doc.font('Helvetica-Bold').fontSize(11).fillColor(hex(C.white))
         .text(tool.rank, M.left + 17, tY + tH / 2 - 7, { lineBreak: false });

      // Tool name
      doc.font('Helvetica-Bold').fontSize(12).fillColor(hex(C.ink))
         .text(tool.name, M.left + 46, tY + 10, { width: UW - 58 });

      // Why — starts exactly after name using measured height
      doc.font('Helvetica').fontSize(10).fillColor(hex(C.mid))
         .text(tool.why || '', M.left + 46, tY + 10 + nameH + 6, { width: UW - 58 });

      doc.y = tY + tH + 4;
    });

    if (narratives.course_recommendation) {
      const crTextH = doc.heightOfString(narratives.course_recommendation, { width: UW - 28, fontSize: 10.5 });
      const crH = Math.max(44, crTextH + 32);
      checkPageBreak(doc, crH + 4, M);
      const crY = doc.y;
      doc.roundedRect(M.left, crY, UW, crH, 8).fill(hex('#EEF5F1'));
      doc.rect(M.left, crY, 4, crH).fill(hex(C.sage));
      doc.font('Helvetica-Bold').fontSize(8).fillColor(hex(C.sage))
         .text('RECOMMENDED COURSE / CERTIFICATION', M.left + 14, crY + 8, { characterSpacing: 0.8, lineBreak: false });
      doc.font('Helvetica').fontSize(10.5).fillColor(hex(C.ink))
         .text(narratives.course_recommendation, M.left + 14, crY + 24, { width: UW - 28 });
      doc.y = crY + crH + 4;
    }
    doc.moveDown(1.2);
    ...
    stepsArr.forEach((step, i) => {
      const stepTextH = doc.heightOfString(step || '', { width: UW - 56, fontSize: 11 });
      const sH = Math.max(46, stepTextH + 32);
      checkPageBreak(doc, sH + 2, M);
      const sY  = doc.y;
      const sBg = i % 2 === 0 ? C.bg2 : C.white;
      doc.rect(M.left, sY, UW, sH).fill(hex(sBg));

      // Number bubble
      doc.circle(M.left + 22, sY + Math.min(23, sH / 2), 16).fill(hex(stepColors[i]));
      doc.font('Helvetica-Bold').fontSize(13).fillColor(hex(C.white))
         .text(`${i+1}`, M.left + (i >= 9 ? 16 : 18), sY + Math.min(23, sH / 2) - 7);
