...
export default async function handler(req, res) {
  ...
  try {
    ...
    // plan: 'brief' (pay what you want, min €1) or 'bundle' (€9.99 fixed)
    const isBundl = plan === 'bundle';
    const PWYW_MIN    = 100; // cents — €1 minimum for the brief
    const PWYW_PRESET = 799; // cents — €7.99 suggested amount shown in the field
    const amount  = isBundl ? 999 : PWYW_PRESET; // cents (preset for PWYW brief)
    const label   = isBundl
    ...
    const desc    = isBundl
      ...
      : `Personalised AI career analysis for ${profession || 'your profession'} — pay what you want`;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'eur',
          // Bundle is a fixed price; the brief is "pay what you want".
          // For PWYW we let the buyer set the amount on Stripe's checkout
          // page via custom_unit_amount, enforcing a €1 minimum.
          ...(isBundl
            ? { unit_amount: amount }
            : {
                custom_unit_amount: {
                  enabled: true,
                  minimum: PWYW_MIN,   // €1
                  preset:  PWYW_PRESET // €7.99 suggested default
                },
              }),
